package com.peng.wechatmoment.interceptor
 ...
 *    desc   : 请求日志拦截器
 */
class LoggingInterceptor @JvmOverloads constructor(private val logger: LoggingInterceptor.Logger = LoggingInterceptor.Logger.DEFAULT) :
    Interceptor {

    @Volatile
    var level: LoggingInterceptor.Level = LoggingInterceptor.Level.NONE
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        val logBody = level == LoggingInterceptor.Level.BODY
        val logHeaders = logBody || level == LoggingInterceptor.Level.HEADERS
        ...
        responseMessage += "<-- " + response.code() +
                ...
                response.request().url() + " (" + tookMs + "ms" + (if (!logHeaders) ", $bodySize body" else "") + ')' + SEPARATOR


//        if (response.code() == 404) {
//            ToastUtils.showShort("后台服务关闭，无法访问")
//        }
    ...
    interface Logger {
        ...
        companion object {
            ...
            val DEFAULT: LoggingInterceptor.Logger = object : LoggingInterceptor.Logger {
